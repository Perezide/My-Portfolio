  ...
  if (!isMobile) {
    return (
      <div className="fixed right-6 top-1/2 transform -translate-y-1/2 z-40 hidden md:flex flex-col items-center space-y-6">
        {/* Connecting line */}
        <div className="absolute w-0.5 h-full bg-gray-300/50 dark:bg-slate-600/50 rounded-full -z-10"></div>
        ...
        {sections.map((section) => (
          ...
          >
            ...
            <div
              className={`w-3 h-3 rounded-full border-2 transition-all duration-200 group-hover:scale-125 ${
                activeSection === section.id
                  ...
                  : 'bg-white dark:bg-slate-800 border-gray-400 dark:border-slate-500 group-hover:border-blue-400'
              }`}
            />
            ...
            <div className="absolute right-8 top-1/2 transform -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none">
              <div className="bg-gray-800 dark:bg-slate-200 text-white dark:text-slate-800 text-sm px-3 py-2 rounded-lg whitespace-nowrap flex items-center space-x-2 shadow-lg">
                <section.icon className="w-3 h-3" />
                <span className="font-medium">{section.label}</span>
              </div>
              <div className="absolute right-0 top-1/2 transform translate-x-1 -translate-y-1/2 w-2 h-2 bg-gray-800 dark:bg-slate-200 rotate-45"></div>
            </div>
          </motion.button>
        ))}
      </div>
                ...
                <section.icon
                  className={`w-6 h-6 transition-all duration-200 ${
                    activeSection === section.id
                      ...
                      : 'text-gray-500 dark:text-slate-400 group-hover:text-blue-400'
                  }`}
                />
